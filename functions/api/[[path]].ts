...
export async function onRequest(context: { request: Request; env: Env; params: any }): Promise<Response> {
  ...
  // 3b. Detailed Square API Diagnostics for Cloudflare Pages
  if (pathname === '/api/square/diagnostics') {
    ...
    const envToCheck = ((body.environment || url.searchParams.get('environment') || squareEnv) as string).toLowerCase().trim();

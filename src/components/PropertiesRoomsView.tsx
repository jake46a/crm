...
export const PropertiesRoomsView: React.FC<PropertiesRoomsViewProps> = ({
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white rounded-sm p-5 border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-sm border border-slate-300 text-slate-700 bg-white hover:bg-slate-50 text-xs font-semibold uppercase tracking-wider transition-colors"
          >
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] px-1.5 py-0.2 rounded-sm font-mono ${
                selectedPropertyId === prop.id ? 'bg-blue-500 text-white font-bold' : 'bg-slate-100 text-slate-600'
              }`}>
      ...
      {selectedProperty && (
        <div className="bg-slate-900 text-white rounded-sm p-6 shadow-xs border border-slate-800">
          <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
            <div className="space-y-3 max-w-2xl">
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                <span className="text-[10px] bg-blue-500/20 text-blue-300 border border-blue-500/30 px-2 py-0.5 rounded-sm font-bold uppercase tracking-wider">
                  {selectedProperty.propertyType}
                </span>
                <span className="text-xs text-slate-400 font-mono">Built {selectedProperty.yearBuilt}</span>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3 pt-1">
                {selectedProperty.wifiNetwork && (
                  <div className="inline-flex items-center gap-2 bg-slate-800 border border-slate-700 px-3 py-1.5 rounded-sm text-xs">
                    <Wifi className="w-3.5 h-3.5 text-blue-400" />
                    <span>Wi-Fi: <strong className="text-slate-200 font-mono">{selectedProperty.wifiNetwork}</strong></span>
                    <span className="text-slate-400 font-mono">({selectedProperty.wifiPassword})</span>
                  </div>
                )}
              ...
              {/* House Rules */}
              <div className="pt-2">
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Coliving House Rules:</p>
                <div className="flex flex-wrap gap-1.5 mt-1.5">
                  {selectedProperty.houseRules.map((rule, idx) => (
                    <span key={idx} className="text-[11px] bg-slate-800 text-slate-300 px-2.5 py-0.5 rounded-sm border border-slate-700">
                      • {rule}
                    </span>
                  ))}
                </div>
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-sm p-4 min-w-[280px] text-xs space-y-2.5">
              <div className="flex items-center justify-between">
                <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Property Management</span>
                <div className="flex items-center gap-2">
                  <button
              ...
              <div>
                <p className="text-[10px] uppercase text-slate-400 font-bold tracking-wider">Owner</p>
                <p className="font-bold text-slate-200 text-sm mt-0.5">{selectedProperty.ownerName}</p>
                <p className="text-slate-400 font-mono text-[11px]">{selectedProperty.ownerPhone}</p>
                <p className="text-slate-400 text-[11px] truncate">{selectedProperty.ownerEmail}</p>
              </div>
              ...
              <div className="pt-2 border-t border-slate-700 flex justify-between items-center">
                ...
                <button
                  ...
                  className="px-2.5 py-1 bg-blue-600 hover:bg-blue-500 text-white rounded-sm font-semibold text-[11px] uppercase tracking-tight flex items-center gap-1 transition-colors"
                >
      ...
      <div className="bg-white rounded-sm p-4 border border-slate-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-sm text-xs font-semibold uppercase tracking-tight transition-colors ${statusFilter === 'all' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
          >
            All ({rooms.length})
          </button>
          <button
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-sm text-xs font-semibold uppercase tracking-tight transition-colors ${statusFilter === 'Available' ? 'bg-blue-700 text-white' : 'bg-blue-50 text-blue-700 hover:bg-blue-100'}`}
          >
          ...
          >
            ...
            <div className="p-4 border-b border-slate-100">
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <h3 className="font-bold text-slate-900 text-sm mt-0.5">{room.name}</h3>
                </div>
              ...
              <div className="flex flex-wrap items-center gap-1.5 mt-2.5">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded-sm bg-slate-100 text-slate-700 border border-slate-200 font-semibold uppercase tracking-tight">
                  {room.isFurnished ? 'Furnished' : 'Unfurnished'}
                </span>
                <span className="text-[10px] px-2 py-0.5 rounded-sm bg-slate-100 text-slate-600 border border-slate-200 font-mono">
            ...
            <div className="p-4 space-y-3 bg-slate-50/50 flex-1">
              {room.status === 'Occupied' ? (
                <div className="bg-white p-3 rounded-sm border border-slate-200 text-xs space-y-1">
                  <div className="flex items-center justify-between">
                  ...
                  <p className="text-slate-500 text-[11px] font-mono">{room.currentTenantPhone}</p>
                  <div className="pt-1.5 border-t border-slate-100 flex justify-between text-[11px]">
                    <span className="text-slate-500">Lease Ends:</span>
                    <strong className="text-slate-900 font-mono">{room.leaseEndDate}</strong>
                  </div>
                </div>
              ) : room.status === 'Available' ? (
                <div className="bg-blue-50/80 p-3 rounded-sm border border-blue-200 text-xs">
                  <p className="font-bold text-blue-900 uppercase tracking-tight text-[11px]">
                  ...
                  <button
                    ...
                    className="mt-2 w-full py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-sm text-xs font-semibold uppercase tracking-wider transition-colors"
                  >
                    Match / Assign Lead
                  </button>
                </div>
                  ...
                  <div className="space-y-1.5 pt-1">
                    {room.turnoverChecklist.map(item => (
                      <div 
                        ...
                        className="flex items-center gap-2 cursor-pointer text-[11px] text-slate-700 hover:text-slate-900 select-none"
                      >
                        {item.isDone ? (
                        ...
                        <span className={item.isDone ? 'line-through text-slate-400' : 'font-medium'}>
                          {item.task}
                        </span>
                      </div>
                    ))}
              ...
              <div className="space-y-1">
                <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Features:</span>
                <div className="flex flex-wrap gap-1">
                  {room.amenities.slice(0, 3).map((am, i) => (
                    <span key={i} className="text-[10px] bg-white border border-slate-200 text-slate-600 px-1.5 py-0.5 rounded-sm font-medium">
                      {am}
                    </span>
                  ))}
                  {room.amenities.length > 3 && (
                    <span className="text-[10px] text-slate-400 self-center">+{room.amenities.length - 3} more</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="p-3 bg-white border-t border-slate-100 flex items-center justify-between text-xs gap-2">
              <select
                ...
                className="text-[11px] bg-slate-50 border border-slate-200 rounded-sm px-2 py-1 text-slate-700 focus:outline-none font-medium"
              >
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="p-1.5 text-slate-500 hover:text-blue-600 hover:bg-slate-100 rounded-sm transition-colors"
                ...
                <button
                  ...
                  className="px-2 py-1 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-sm font-medium text-[11px] transition-colors uppercase tracking-tight"
                >
                  Edit
                </button>
                {onDeleteRoom && (
